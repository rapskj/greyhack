authentication = function()
	...
	Print("\n"+t.o+"[arsys authentication "+ar_version+"]"+C.e)
...
Commands["exit"] = {"Name": "exit","Description": "Exits from arsys.","Args": "","Shell":false}
Commands["exit"]["Run"] = function(args,pipe)
	return exit("Exiting arsys...")
end function
...
Commands["manual"]["Run"] = function(args,pipe)
	...
	while 1
		...
		while 1
			...
			if not metaLib then exit("Thanks for using arsys")
	...
	system_shell = function()
		...
		if args[0] == "secure" then
			...
			if not globals.usr == "root" then
				Print(t.e+"arsys cannot be sure that this command worked due to no root access"+C.e)	
			end if
			
			securesys(globals.comp)

			Print(t.s+"arsys has secured this machine.")
